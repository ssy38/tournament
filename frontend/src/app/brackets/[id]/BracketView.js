...
export default function BracketView(props) {
  ...
  return (
    <>
      <div
        ...
        } h-[calc(100vh-140px)] w-screen flex-shrink overflow-scroll px-12 py-10`}
      >
        <div
          style={{ height: height + "px" }}
          className={`flex flex-row rounded-lg pr-8 ${width} ${
            2 ** numRounds <= 8 && "xl:w-screen"
          }`}
        >
          {createBracket()}
          <div className="inline-flex w-full flex-col justify-around align-middle">
            <div
              className={`b-0 relative flex flex-col items-center border-t-2 border-white bg-transparent`}
            >
              <div
                className={`${
                  winner_id ? "bg-blue-700" : "bg-slate-900"
                } absolute right-0 flex h-24 
                     w-[80%] -translate-y-1/2 transform items-center rounded-xl border-2 text-lg`}
              >
                {teams.find((team) => team.id === winner_id) !== undefined && (
                  <div className="flex h-full w-[20%] min-w-max flex-shrink-0 items-center justify-center border-r-2 text-slate-400">
                    {teams.find((team) => team.id === winner_id).seed}
                  </div>
                )}
                <div className="line-clamp-3 px-2">
                  {teams.find((team) => team.id === winner_id) !== undefined &&
                    teams.find((team) => team.id === winner_id).name}
                </div>
                <div className="absolute top-0 line-clamp-2 -translate-y-[140%] transform px-2">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                    strokeWidth={1.5}
                    className="h-8 w-8 stroke-yellow-500"
                  >
                    <path
                      strokeLinecap="round"
            ...
            match={
              extraMatches !== 0
                ? extraMatches.find((match) =>
                    seedOrder[round][i].includes(match.expected_seed),
                  )
                : matches.find((match) =>
                    seedOrder[round][i].includes(match.expected_seed),
                  )
  ...
  function MatchBlock({ round, teams, match }) {
    ...
    function setWinner(team) {
      ...
      if (winnerMatch.team1 === null || winnerMatch.team2 === null) return;
    ...
    return (
      <div
        ...
        } b-0 relative flex flex-col items-center border-y-2 border-r-2 border-white bg-transparent`}
      >
        <div
          ...
          } h-20 w-[80%] -translate-y-1/2 transform rounded-xl border-2 text-lg transition-all`}
        >
          {team1 !== undefined && (
            <div className="flex h-full w-[15%] min-w-max flex-shrink-0 items-center justify-center border-r-2 text-sm text-slate-400">
              {team1.seed}
            </div>
          )}
