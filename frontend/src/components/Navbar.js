...
export function NavButton({label}) {
  return <button className="flex items-center justify-center bg-sky-600 rounded-xl px-7 py-2 text-sm font-medium w-fit">
    {label}
  </button>
}
...
export default function Navbar() {
  ...
  return <>
    <nav className="sticky top-0 w-full bg-slate-900 z-50">
      <div className="flex flex-wrap justify-between items-center p-4">
        ...
        <div className={`${!open && "hidden"} w-full mt-4 md:m-0 md:block md:w-auto`}>
            <ul className="flex p-3 flex-col w-full rounded-lg text-m mr-5 bg-slate-800 space-y-2 md:p-0 md:flex-row md:space-x-8 md:space-y-0 md:bg-slate-900">
                ...
                <li>
                    <NavLink href="/brackets" label="Brackets" active={active}></NavLink>
                </li>
                <li>
